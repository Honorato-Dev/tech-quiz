...
import quizDataDjango from '@/utils/quizzDataDjango';
import Layout from '@/components/Layout';



function DjangoScreen() {
  ...
  const handleNextQuestion = () => {
    if (selected === quizDataDjango[currentQuiz].correct) {
      setScore(score + 1);
    }
    setSelected(null);
  ...
  return (
    <Layout title='Java'>
      ...
      {currentQuiz < quizDataDjango.length ? (
        <div className='flex flex-col text-center mx-auto max-w-screen-md bg-white bg-opacity-80  rounded-sm p-6'>
          ...
          <p className='text-lg lg:text-xl mb-6 mt-4'>{quizDataDjango[currentQuiz].question}</p>
          <div className=''>{quizDataDjango[currentQuiz].image}</div>
          <ul className='flex flex-col text-left'>
            {quizDataDjango[currentQuiz].options.map((option) => (
        ...
        <div className='justify-content  text-center'>
          <h2 className='text-lg md:text-xl lg:text-2xl mb-6'>Você respondeu <span className='fond-bold text-2xl md:text-3xl lg:text-4xl'>{score}/{quizDataDjango.length}</span> questões corretamente</h2>
...
export default DjangoScreen;
DjangoScreen.auth=true;