...
import quizDataRust from '@/utils/quizzDataRust';
...
function RustScreen() {
  ...
  const handleNextQuestion = () => {
    if (selected === quizDataRust[currentQuiz].correct) {
      setScore(score + 1);
    }
    setSelected(null);
  ...
  return (
    <Layout title='Java'>
      ...
      {currentQuiz < quizDataRust.length ? (
        <div className='flex flex-col text-center mx-auto max-w-screen-md  bg-opacity-80  rounded-sm p-6'>
          ...
          <p className='text-lg lg:text-xl mb-6 mt-4'>{quizDataRust[currentQuiz].question}</p>
          <div className=''>{quizDataRust[currentQuiz].image}</div>
          <ul className='flex flex-col text-left'>
            {quizDataRust[currentQuiz].options.map((option) => (
        ...
        <h2 className='text-lg md:text-xl lg:text-2xl mb-6'>Você respondeu <span className='fond-bold text-2xl md:text-3xl lg:text-4xl'>{score}/{quizDataRust.length}</span> questões corretamente</h2>
