...
import quizDataMongoDb from '@/utils/quizzDataMongoDb';
import Layout from '@/components/Layout';



function MongoDbScreen() {
  ...
  const handleNextQuestion = () => {
    if (selected === quizDataMongoDb[currentQuiz].correct) {
      setScore(score + 1);
    }
    setSelected(null);
  ...
  return (
    <Layout title='Java'>
      ...
      {currentQuiz < quizDataMongoDb.length ? (
        <div className='flex flex-col text-center mx-auto max-w-screen-md bg-white bg-opacity-80  rounded-sm p-6'>
          ...
          <p className='text-lg lg:text-xl mb-6 mt-4'>{quizDataMongoDb[currentQuiz].question}</p>
          <div className=''>{quizDataMongoDb[currentQuiz].image}</div>
          <ul className='flex flex-col text-left'>
            {quizDataMongoDb[currentQuiz].options.map((option) => (
        ...
        <h2 className='text-lg md:text-xl lg:text-2xl mb-6'>Você respondeu <span className='fond-bold text-2xl md:text-3xl lg:text-4xl'>{score}/{quizDataMongoDb.length}</span> questões corretamente</h2>
