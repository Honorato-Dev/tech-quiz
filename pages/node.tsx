    ...
    <Layout title='Java'>
      ...
      {currentQuiz < quizDataNode.length ? (
        <div className='flex flex-col text-center mx-auto max-w-screen-md  bg-opacity-80  rounded-sm p-6'>
          ...
          <p className='text-lg lg:text-xl mb-6 mt-4'>{quizDataNode[currentQuiz].question}</p>
          <div className=''>{quizDataNode[currentQuiz].image}</div>
        ...
        <div className='justify-content  text-center'>
          <h2 className='text-lg md:text-xl lg:text-2xl mb-6'>Você respondeu <span className='fond-bold text-2xl md:text-3xl lg:text-4xl'>{score}/{quizDataNode.length}</span> questões corretamente</h2>
