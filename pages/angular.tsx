...
import quizDataAngular from '@/utils/quizzDataAngular';
...
function AngularScreen() {
  ...
  const handleNextQuestion = () => {
    if (selected === quizDataAngular[currentQuiz].correct) {
      setScore(score + 1);
    }
    setSelected(null);
  ...
  return (
    <Layout title='Java'>
      ...
      {currentQuiz < quizDataAngular.length ? (
        <div className='flex flex-col text-center mx-auto max-w-screen-md  bg-opacity-80  rounded-sm p-6'>
          ...
          <p className='text-lg lg:text-xl mb-6 mt-4'>{quizDataAngular[currentQuiz].question}</p>
          <div className=''>{quizDataAngular[currentQuiz].image}</div>
          <ul className='flex flex-col text-left'>
            {quizDataAngular[currentQuiz].options.map((option) => (
        ...
        <h2 className='text-lg md:text-xl lg:text-2xl mb-6'>You answered <span className='fond-bold text-2xl md:text-3xl lg:text-4xl'>{score}/{quizDataAngular.length}</span> Questions correctly</h2>
...
export default AngularScreen;
AngularScreen.auth=true;