...
import quizDataCsharp from '@/utils/quizzDataCsharp';
import Layout from '@/components/Layout';



function CsharpScreen() {
  ...
  const handleNextQuestion = () => {
    if (selected === quizDataCsharp[currentQuiz].correct) {
      setScore(score + 1);
    }
    setSelected(null);
  ...
  return (
    <Layout title='Java'>
      ...
      {currentQuiz < quizDataCsharp.length ? (
        <div className='flex flex-col text-center mx-auto max-w-screen-md bg-white bg-opacity-80  rounded-sm p-6'>
          ...
          <p className='text-lg lg:text-xl mb-6 mt-4'>{quizDataCsharp[currentQuiz].question}</p>
          <div className=''>{quizDataCsharp[currentQuiz].image}</div>
          <ul className='flex flex-col text-left'>
            {quizDataCsharp[currentQuiz].options.map((option) => (
        ...
        <div>
          <h2>You answered {score}/{quizDataCsharp.length} questions correctly</h2>
...
export default CsharpScreen;
CsharpScreen.auth=true;